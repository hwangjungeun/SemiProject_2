...
//			String userid = request.getParameter("userid"); // 주소창에 넘어온 userid
		...
		*/		
				...
				// 최근본제품 목록들을 페이지바를 이용한 페이징 처리하여 조회(select)해오기 
				...
				int totalPage = pdao.getRecentTotalPage(userid); // 특정 userid를 넘김. // ##################################뿌잉##################################
			//	System.out.println("~~~ 확인용 totalPage : " + totalPage);
				
				
				// **** [맨처음][이전] 만들기 **** //
				if(pageNo != 1) {
					pageBar += "<li class='page-item'><a class='page-link' href='recentViewProduct.go?currentShowPageNo=1&userid="+userid+"'><span class='text-dark' aria-hidden='true'>&lt;&lt;</span></a></li>"; // ##################################뿌잉##################################
					pageBar += "<li class='page-item'><a class='page-link' href='recentViewProduct.go?currentShowPageNo="+(pageNo-1)+"&userid="+userid+"'><span class='text-dark' aria-hidden='true'>&lt;</span></a></li></a></li>"; // ##################################뿌잉##################################
				}
				...
				while( !(loop > blockSize || pageNo > totalPage) ) { // loop가 11이 되어지면 탈출. // 전체페이지수보다 많으면 탈출.
					...
					if( pageNo == Integer.parseInt(currentShowPageNo) ) {
						...
						pageBar += "<li class='page-item'><a class='page-link' href='recentViewProduct.go?currentShowPageNo="+pageNo+"&userid="+userid+"'><span class='text-dark'>"+pageNo+"</span></a></li>"; // ##################################뿌잉##################################
					}
				...
				if( pageNo <= totalPage ) { // while문의 탈출조건인 pageNo > totalPage가 아닌 경우
					pageBar += "<li class='page-item'><a class='page-link' href='recentViewProduct.go?currentShowPageNo="+pageNo+"&userid="+userid+"'><span class='text-dark' aria-hidden='true'>&gt;</span></a></li>"; // ##################################뿌잉##################################
					pageBar += "<li class='page-item'><a class='page-link' href='recentViewProduct.go?currentShowPageNo="+totalPage+"&userid="+userid+"'><span class='text-dark' aria-hidden='true'>&gt&gt;</span></a></li>"; // ##################################뿌잉##################################
				}
